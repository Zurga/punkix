...
<% end %><%= for %{assoc_fun: :belongs_to} = assoc <- schema.assocs do %>      add <%= inspect(assoc.key) %>, references(<%= inspect(assoc.assoc_table) %>, on_delete: :nothing<%= if schema.binary_id do %>, type: :binary_id<% end %>)<%= if assoc.required do %>, null: false<% end %>
<% end %>
